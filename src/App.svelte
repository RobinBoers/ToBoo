			...
			{#each issues as item, i}
				...
				{#if item !== null && item !== ""}
					...
					{#if item.state === "open" || showFinished === true}
						...
						<li class={clickedIssues[i] === true ? 'selected' : ''} on:click|self={() => {
							if(clickedIssues[i] !== true) clickIssue(item);
						}}>
							<span on:click={() => {
								if(clickedIssues[i] === true) clickIssue(item);
							}} class="issue-head">
							...
							{#if clickedIssues[i] === true}
								<span class="issue-body">
									...
									{:else}
										<!-- <i>Empty</i><br> -->
									{/if}
									<button on:click={() => {window.open(item.html_url, "_blank")}}><i class="fab fa-github"></i> View on GitHub</button>
								</span>
							{/if}
						</li>
